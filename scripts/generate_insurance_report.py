...
def main():
    ...
    add_paragraph_zh(doc,
        ...
        '多次赔付总额、老年疾病长期护理等核心维度上均显著优于内地三款产品。内地产品虽然在理赔便利性、'
        '医院范围和本地服务响应方面具备优势，但其保额固定、疾病定义相对严格、多次赔付空间有限，'
        '长期保障价值明显弱于保诚。对于追求长期高杠杆保障、美元资产配置和抗通胀能力的客户，'
        '保诚 CIM3/BCIM3 是更优选择。')
    ...
    table_data = [
        ...
        ['保障疾病范围', '56种严重疾病 + 127种病况', '120种重疾（可选40种轻症+20种中症）', '120种重疾（可选40种轻症+20种中症）', '120种重疾 + 20种中症 + 40种轻症'],
        ['疾病定义宽松度', '宽松：中风不要求180天后遗留障碍，部分早期癌症/甲状腺癌定义更友好', '较严格：遵循内地重疾定义规范，部分病种需达特定状态', '较严格：遵循内地重疾定义规范', '较严格：遵循内地重疾定义规范'],
        ['重疾赔付次数/总额', '严重疾病1次 + 多重危疾延伸，总额最高达保额1000%', '基础单次；可选6组额外5次（限88岁前）', '基础单次；可选2-3次不分组', '单次赔付'],
        ['癌症多次赔付', '癌症最高累计200%保额', '基础单次；多次赔需癌症单独分组', '基础单次；二次重疾赔120%（须为其他重疾）', '单次'],
        ['心梗/中风多次赔付', '心脏病/中风各最高累计200%保额', '多次赔需跨组', '可选二次重疾可分别赔付', '不赔'],
    ...
    add_table_zh(doc, 10, 5, table_data)
    ...
    table_data = [
        ...
        ['首次确诊重疾', '赔付50万（基础保额）', '赔付50万', '赔付50万', '赔付50万'],
    ...
    add_heading_zh(doc, '3.1 保额增长与抗通胀能力', level=2)
    add_paragraph_zh(doc,
    ...
    add_heading_zh(doc, '3.2 疾病定义宽松度与理赔门槛', level=2)
    add_paragraph_zh(doc,
